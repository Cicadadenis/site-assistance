...
apt purge -y mariadb* mysql* libmariadb* 2>/dev/null || true
rm -rf /etc/mysql /var/lib/mysql /var/log/mysql /run/mysqld ~/.my.cnf 2>/dev/null || true
...
tee /etc/nginx/sites-available/webasyst > /dev/null <<EOF
server {
    listen 80;
    server_name _;
    root $TARGET_DIR;
...
ln -sf /etc/nginx/sites-available/webasyst /etc/nginx/sites-enabled/
